...
int acc_calibration_value = -235;                           //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 42;                                      //Gain setting for the P-controller (15)
...
/*
 X - enter PID adjust mode
 x - exit PID adjust mode
 W - increase P by the latest digit received
 w - decrease P by the latest digit received
 U - increase I by the latest digit/100 received
 u - decrease I by the latest digit/100 received
 V - increase D by the latest digit received
 v - decrease D by the latest digit received
 L,R,F,B - go left, right, forward, backward, 
 S - stop
 r - output all the settings
 */
bool pid_adjust_mode = false;
byte last_received_digit = 0;
...
void setup(){
  ...
  pinMode(3, OUTPUT);  //DIR y                                              
  pinMode(5, OUTPUT);  //step x                                             
  pinMode(6, OUTPUT);  //step y                                              
  pinMode(2, OUTPUT);  //DIR x 
  pinMode(13, OUTPUT);                                                      //Configure digital poort 6 as output
  pinMode(8, OUTPUT); //Enabled
  digitalWrite(8, HIGH);                                                    //Disable motor drivers
...
void loop(){
  ...
  /* battery voltage detection is disabled
  ...
  */
  ...
  if(start == 0 && angle_acc > -0.5&& angle_acc < 0.5){                     //If the accelerometer angle is almost 0
    ...
    digitalWrite(8, LOW);                                                   //Enable motor drivers
  }
  ...
  if(angle_gyro > 30 || angle_gyro < -30 || start == 0 || low_bat == 1){    //If the robot tips over or the start variable is zero or the battery is empty
    ...
    if(received_byte == (byte)'r'){                                         //If r char is received print current settings
      Serial.print("\r\nP: "); Serial.print(pid_p_gain);
      Serial.print("\r\nI: "); Serial.print(pid_i_gain);
      Serial.print("\r\nD: "); Serial.print(pid_d_gain);
      Serial.print("\r\nS: "); Serial.println(turning_speed);
      received_byte = 0x00;                                                 //Reset received byte
    }
  }
  ...
  else if(received_byte == (byte)'S'){                                      //Slowly reduce the setpoint to zero if no foreward or backward command is given
  ...
  else if(received_byte == (byte)'F'){                                      //If the third bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  else if(received_byte == (byte)'B'){                                      //If the forth bit of the receive byte is set change the left and right variable to turn the robot to the right
  ...
  else if(received_byte == (byte)'L'){                                      //If the first bit of the receive byte is set change the left and right variable to turn the robot to the left
    pid_output_left += turning_speed;                                       //Increase the left motor speed
    pid_output_right -= turning_speed;                                      //Decrease the right motor speed
  }
  else if(received_byte == (byte)'R'){                                      //If the second bit of the receive byte is set change the left and right variable to turn the robot to the right
    pid_output_left -= turning_speed;                                       //Decrease the left motor speed
    pid_output_right += turning_speed;                                      //Increase the right motor speed
  }
  else if(received_byte == (byte)'X'){                                      //If X is received enter PID adjust mode
    pid_adjust_mode = true;  
    received_byte = 0x00;                                                   //Reset received byte
  }
  else if(received_byte == (byte)'x'){                                      //If x is received exit PID adjust mode
    pid_adjust_mode = false;
    received_byte = 0x00;                                                   //reset received byte 
  }
  else if(pid_adjust_mode == true){                                         //If PID adjust mode is active, make PID commanded adjustments
    if(received_byte <= (byte)'9' && received_byte >=(byte)'0'){            //If 0-9 char is received save it 
      last_received_digit = received_byte - '0';                            //Store received digit converting it from char to numeric
    }
    else if(received_byte == (byte)'W'){                                    //If W is received, increase P by last_received_digit
      pid_p_gain += last_received_digit;   
    }
    else if(received_byte == (byte)'w'){                                    //If w is received, decrease P by last_received_digit
      pid_p_gain -= last_received_digit;     
    }
    else if(received_byte == (byte)'U'){                                    //If U is received, increase I by last_received_digit/100  
      pid_i_gain += (float)last_received_digit/100; 
    }
    else if(received_byte == (byte)'u'){                                    //If u is received, decrease I by last_received_digit/100    
      pid_i_gain -= (float)last_received_digit/100; 
    }      
    else if(received_byte == (byte)'V'){                                    //If V is received, increase D by last_received_digit
      pid_d_gain += last_received_digit;   
    }
    else if(received_byte == (byte)'v'){                                    //If v is received, decrease D by last_received_digit
      pid_d_gain -= last_received_digit;     
    }
...
ISR(TIMER2_COMPA_vect){
  ...
  else if(throttle_counter_left_motor == 1)PORTD |= 0b00100000;             //Set output 7 high to create a pulse for the stepper controller
  else if(throttle_counter_left_motor == 2)PORTD &= 0b11011111;             //Set output 7 low because the pulse only has to last for 20us 
  ...
  else if(throttle_counter_right_motor == 1)PORTD |= 0b01000000;            //Set output 5 high to create a pulse for the stepper controller
  else if(throttle_counter_right_motor == 2)PORTD &= 0b10111111;            //Set output 5 low because the pulse only has to last for 20us
}
